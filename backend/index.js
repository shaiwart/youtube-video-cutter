...
function generateOriginalAudioFilePath(videoId, audioFormat) {
	return path.join(__dirname, "input", `${videoId}.${audioFormat}`);
}

async function downloadAudio(url, audioFormat) {
	console.log(`... Started downloading audio ...`);
	const audioFilePath = generateOriginalAudioFilePath(getVideoIdByURL(url), audioFormat);
	await youtubedl(url, {
		extractAudio: true,
...
async function cutAudio(inputFilePath, outputFilePath, startTime, endTime, audioFormat) {
	return new Promise((resolve, reject) => {
		let codecOption = '';
		if (audioFormat === 'mp3') {
			codecOption = '-c:a libmp3lame';
...
app.post('/download-audio-only', async (req, res) => {
	try {
		const { url, startTime, endTime, audioFormat = 'mp3' } = req.body;
		if (!url || !startTime || !endTime)
			return res.status(400).json({ error: 'Missing required fields.' });
		...
		const outputExt = audioFormat === 'm4a' ? 'm4a' : 'mp3';
		const outputFileName = `audio_${videoId}_${startTime}_${endTime}.${outputExt}`;
		const outputFilePath = path.join(__dirname, "output", outputFileName);
